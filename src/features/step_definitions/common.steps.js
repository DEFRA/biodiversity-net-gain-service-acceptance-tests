...
async function confirmLegalAgreementDetails () {
  await pages['check-legal-agreement-details'].acceptBtn.waitForExist({ timeout: 5000 })
...
Then('I should see the error {string}', async (message) => {
  // Wait for the error message element to exist with a timeout
  await pages['legal-agreement-upload'].errorMsg.waitForExist({ timeout: 5000 })
  
  const actualText = await pages['legal-agreement-upload'].errorMsg.getText();

   // Ensure the element is displayed
   const isDisplayed = await pages['legal-agreement-upload'].errorMsg.isDisplayed();
   if (!isDisplayed) {
     throw new Error("Error message element is not displayed.");
   }

  await expect(actualText).toContain(message);
})
...
async function completeAddDevelopmentInfoSection (lpa, applicationNumber, projectName, journey) {
  switch (journey) {
    case 'credits': {
      ...
      await expect(pages['credits-purchase-tasklist'].addDevelopmentInformationStatus).toHaveText('Completed')
      break
    }
    case 'allocation': {
